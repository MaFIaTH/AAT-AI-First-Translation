# JFA Episode 3: Turnabout Big Top — Localization Reference

This reference guide details names spelling dictionary, terminology standards, pronoun choices, interpersonal registers, and speech quirks specific to **JFA Episode 3: Turnabout Big Top**.

---

## 1. Names Spelling Dictionary (EP3 Only)

| English Name | Thai Translation | Tagged / Brackets Format | Notes |
|---|---|---|---|
| Acro | อัคโคร | `<อัคโคร>` | |
| Bat | แบท | `<แบท>` | |
| Regina Berry | เรจิน่า เบอร์รี่ | `<เรจิน่า เบอร์รี่>` | |
| Russell Berry | รัสเซล เบอร์รี่ | `<รัสเซล เบอร์รี่>` | Ringmaster |
| Max Galactica | แม็กซิมิเลี่ยน กาแลกติก้า | `<แม็กซิมิเลี่ยน กาแลกติก้า>` (Nick: `<แม็กซิมิเลี่ยน>`, `<กาแลกติก้า>`, or Maya's casual nickname `คุณ<แม็ก>`) | Standardized with tone mark |
| Moe | โม | `<โม>` (Lawrence Moe Curls: `<ลอว์เรนส์ โม เคิร์ลล์>`) | |
| Money (monkey) | มันนี่ | `<มันนี่>` | |
| Regent (tiger) | รีเจ้นท์ | `<รีเจ้นท์>` | |
| Benjamin Woodman | เบนจามิน วูดแมน | `<เบนจามิน วูดแมน>` (Nick: `<เบ็น>`) | Standardized with Mai Tai Khu |
| Léon (lion) | ลีออน | `<ลีออน>` | |
| Trilo Quist | ทริโล ควิส | `<ทริโล ควิส>` (Nick: `<ทริโล>`) | |

---

## 2. Terminology Standards (EP3 Only)
- **`Ringmaster`** $\rightarrow$ **`หัวหน้าคณะ`** (standard Thai circus vocabulary; refined from raw "ผู้จัดการละครสัตว์").
- **`Berry Big Circus`** $\rightarrow$ **`คณะละครสัตว์<เบอร์รี่บิ๊ก>`** (standardized spelling with tone mark and name tags).
- **`Big Top`** $\rightarrow$ **`เต็นท์ใหญ่<เบอร์รี่>`** or **`เต็นท์ใหญ่`** (refined from raw "กระโจมใหญ่").
- **`Troupe member`** $\rightarrow$ **`ลูกคณะ`**.
- **`Puppet / Ventriloquist doll`** $\rightarrow$ **`หุ่น`** or **`หุ่นพากย์`** (refined from raw "ตุ๊กตา").
- **`Attorney Badge`** $\rightarrow$ **`ตรา`** (refined from raw "เข็มกลัด").

---

## 3. Pronouns & Relationship registers (EP3 Only)

| Character | Speaking To | Self-Pronoun (Thai) | Address/Refers to Other As | Tone / Particles / Notes |
|---|---|---|---|---|
| **Regina Berry** | Phoenix Wright | **ฉัน** (refined from "หนู") | `คุณ` / `คุณทนาย` | sheltered, childlike circus-girl. Drops formal `ค่ะ/คะ` for childlike/spoiled particles (**นะ**, **น่ะ**, **ล่ะ**, **ซะหน่อย**, **เหรอ**). Refers to animals with **มัน / ตัว** instead of human pronouns. |
| **Regina Berry** | Maya Fey | **ฉัน** | `เธอ` | Peer dynamic. |
| **Max Galactica** | Phoenix Wright | **Stage:** `ฉัน` / `ผม`<br>**True:** `ผม` / `ข้อย` (Isan) | **Stage:** `คุณพี่` / `เจ้าหัวเม่น`<br>**True:** `คุณพี่` / `ซุมเจ้า` (you guys) | **Stage:** Flamboyant, romantic (uses **จ้ะ**, **จ๊ะ**, **จ๋า**, calls Maya **ยาหยี**).<br>**True:** slips into Isan dialect under stress (uses **แหน่**, **บ่**, **อีหลี**, **คักแท้**). |
| **Moe** | Phoenix / Others | `ฉัน` | `นาย` / `พวก` (Phoenix)<br>`เธอ` / `น้องหนู` (Regina) | Energetic clown register. Desperately tries to be funny. Laugh style: **อะฮ่า! อะฮ่า! อะฮ่า!**. |
| **Benjamin Woodman** | Court / Clients | `ผม` | `คุณ` | Highly polite register. Ends sentences with **ครับ**. |
| **Benjamin Woodman** | Trilo Quist | `ฉัน` | `นาย` | Drops politeness and "ครับ" due to intense stress/insecurity in this relationship. |
| **Trilo Quist** | Ben | `ฉัน` / `ทริโล` | `แก` / `ไอ้<เบ็น>` | Loud, rude, and aggressive. Alter-ego dynamic. |
| **Trilo Quist** | Phoenix / Others | `ฉัน` / `คุณควิส` | `แก` | Aggressive, uses harsh Thai insults (e.g., **ไอ้หน้าตูดหมึก**, **ไอ้ทนายค้าความ**, **ไอ้แก่พิลึก**). |

---

## 4. Character Speech Quirks & Localization (EP3 Only)

### A. Pearl Fey's Mishearing quirk
Pearl is sheltered and mishears modern/urban words:
- **English:** *magician* $\rightarrow$ **Thai:** `นักมารยากล` (mar-ya-kol, a play combining "deceitful trickery" and "magic").
- **English:** *high-speed train* $\rightarrow$ **Thai:** `รถไฟความเลวสูง` ("high-evil train" instead of "ความเร็วสูง").

### B. Moe's Puns & Wordplay
Moe's humor must be localized into Thai cultural contexts rather than translated literally:
- **Phuket (ผู้เก็ต) Pun:** Playing on the homophone **"ผู้เก็ต"** (*those who get the joke*): `"มีแต่คน 'ภูเก็ต' เท่านั้นแหละที่จะเข้าใจ... ภู 'เก็ต' ไหม?"`
- **Porridge (โจ๊ก) Pun:** Playing on **ตัวโจ๊ก** (*Joker / Porridge*) and **ข้าวต้ม** (*Boiled Rice / Serious work*): `"ถึงฉันจะเป็นตัวโจ๊ก แต่เรื่องงานฉันข้าวต้มนะ!"`
- **Win Motorbike Pun:** Comparing a spirit acolyte vest to a motorcycle taxi vest: `"เกือบนึกว่าเป็นร่างทรงวิญ... มอเตอร์ไซต์!"`

### C. Trilo's Aggressive Clichés
Trilo uses harsh, colorful Thai insults to assert dominance:
- *Raw:* `ไอ้หน้าก้น` $\rightarrow$ *Refined:* **`ไอ้หน้าตูดหมึก`**
- *Raw:* `ไอ้ทนายฉวยโอกาส` $\rightarrow$ *Refined:* **`ไอ้ทนายค้าความ`** (derogatory term for a crooked lawyer)
- *Raw:* `ตาแก่พิลึก` $\rightarrow$ *Refined:* **`ไอ้แก่พิลึก`**
- **Wood Pun:** Retorting to Ben's idiom "อย่าพูดขวานผ่าซาก" (don't speak bluntly/split logs with an axe): `"ฉันทำมาจากไม้นะเฮ้ย!!"` (*I'm made of wood!*).
- **Madonna Nickname:** Localized from "Madonna" to the ultimate Thai epic standard of beauty: **`"นางสีดาของฉัน"`**.

### D. Phoenix's Name Pun
- Plays on Wright sounding like "Right/Correct" (English) or "ร้าย" (Rai/bad/naughty) in Thai: `"ก็เพราะว่าผมคือฟีนิกซ์ "ร้าย""`.
